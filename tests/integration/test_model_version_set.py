...
from tests.integration.config import (
    ...
    IT_TC_TEST_PROJECT_OCID
)



SAMPLE_PAYLOAD = dict(
    compartment_id=IT_NETWORKS_COMPARTMENT_OCID,  # network
    name=f"adstest-mvs-{str(uuid4())}",
    project_id=IT_TC_TEST_PROJECT_OCID,
...
class TestModelVersionSet:
    ...
    def test_create_delete(self):
        """Tests deleting created MVS."""
        SAMPLE_PAYLOAD["name"] = f"adstest-delete-mvs-{str(uuid4())}"
        mvs = ModelVersionSet(**SAMPLE_PAYLOAD).create()
        assert mvs.id
        assert mvs.status == mvs.LIFECYCLE_STATE_ACTIVE
        mvs.delete()
        assert mvs.status == mvs.LIFECYCLE_STATE_DELETED
